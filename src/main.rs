...
impl GameState {
    ...
    fn update(&mut self, pressed_keys: &HashSet<u32>) {
        if pressed_keys.contains(&gdk::keys::constants::w) && self.paddle1_y > 0 {
            self.paddle1_y -= PADDLE_SPEED;
        }
        if pressed_keys.contains(&gdk::keys::constants::s) && self.paddle1_y < HEIGHT - PADDLE_HEIGHT {
            self.paddle1_y += PADDLE_SPEED;
        }
        if pressed_keys.contains(&gdk::keys::constants::Up) && self.paddle2_y > 0 {
            self.paddle2_y -= PADDLE_SPEED;
        }
        if pressed_keys.contains(&gdk::keys::constants::Down) && self.paddle2_y < HEIGHT - PADDLE_HEIGHT {
            self.paddle2_y += PADDLE_SPEED;
        }
    ...
    fn paddle1_up(&mut self) {
        if self.paddle1_y > 0 {
            self.paddle1_y -= PADDLE_SPEED;
        }
    }

    fn paddle1_down(&mut self) {
        if self.paddle1_y < HEIGHT - PADDLE_HEIGHT {
            self.paddle1_y += PADDLE_SPEED;
        }
    }
}
...
fn main() {
    ...
    let pressed_keys: Rc<RefCell<HashSet<u32>>> = Rc::new(RefCell::new(HashSet::new()));

    let pressed_keys_press = pressed_keys.clone();
    window.connect_key_press_event(move |_, key| {
        pressed_keys_press.borrow_mut().insert(key.keyval().into_glib());
        Inhibit(false)
    });
    let pressed_keys_release = pressed_keys.clone();
    window.connect_key_release_event(move |_, key| {
        pressed_keys_release.borrow_mut().remove(&key.keyval().into_glib());
        Inhibit(false)
    });
    ...
    glib::timeout_add_local(Duration::from_millis(16), move || {
        let pressed_keys_clone = pressed_keys.borrow().clone();
        state.borrow_mut().update(&pressed_keys_clone);
